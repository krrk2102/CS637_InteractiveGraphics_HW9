...
vec4 product(vec4 left, vec4 right)
{
    return vec4(left.x*right.x, left.y*right.y, left.z*right.z, left.w*right.w);
}

// Following few functions are for Perlin Noise generation. 
//
...
float synthesisNoise(vec4 freq, float n)
{
    float noise = 0;
    for (float i = 1; i < n; i *= 1.5) {
        noise += pow(2, -i) * cnoise(vec4(pow(2, i)*freq.x, pow(2, i)*freq.y, pow(2, i)*freq.z, pow(2, i)*freq.w));
    }
    return noise;
}

vec4 textureMap(float a)
{
    float factor = smoothstep(0.05, 0.1, a);
    return normalize((1-factor)*vec4(0.05, 0.05, 0.1, 1)+factor*vec4(0.9, 0.9, 0.9, 1));
}
...
void main()
{
    vec4 textureColor;
    if (texture) {
        textureColor = textureMap(mod(sqrt(pow(TexCoordPass.x,2)+pow(TexCoordPass.z,2))+synthesisNoise(TexCoordPass, 10), 0.5));
        //textureColor = textureMap(mod(TexCoordPass.x+synthesisNoise(TexCoordPass, 4), 0.6));
    } else {
        textureColor = vec4(0.5, 0.5, 0.5, 1);
    }
    ...
    diffuse1 = (Kd1 * 2.5 * product(textureColor, light1_diffuse_product)).xyz;
    diffuse2 = (Kd2 * 2.5 * product(textureColor, light2_diffuse_product)).xyz;
